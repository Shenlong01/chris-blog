...
def register():
    ...
    if form.validate_on_submit():
        ...
        register_password = generate_password_hash(password, method='pbkdf2:sha256', salt_length=8)
        check_email = User.query.filter_by(email=form.email.data).first()
        if check_email:
            flash("Sorry but this email is already in use.")
...
def show_post(post_id):
    ...
    form = CommentForm()
    # post_comments = Comment.query.get(post_id).all
